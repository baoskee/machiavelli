...
/**
 * Should be responsible for managing fields in error messages
 * @param spec
 * @returns {Schema}
 * @constructor
 */
...
function validateThrow(data) {
  for (var key in this.spec) {
    if (this.spec.hasOwnProperty(key)) {
      ...
      } else if (specValue instanceof Schema) {
        specValue.validate(data[key], function (err) {
          if (err) throw err;
        });
      } else {
    ...
    if (this.spec.hasOwnProperty(key)) {
      ...
      if (specValue instanceof Field) {
        if (data[key] != undefined) {
          result[key] = data[key];
        } else if (specValue.default != undefined) {
          result[key] = specValue.getDefault();
        }
      } else if (specValue instanceof Schema) {
        result[key] = specValue.capture(data[key]);
      } else { /* not an instance of Field, assumes it is another nested object */
